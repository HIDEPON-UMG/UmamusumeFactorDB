...
- **Pass 0 で「OCR conf + 緑名前テンプレ conf」の max を best_green_score に採用**: 同 uma 内に row=1 col=0 と row=2 col=0 の両方が color=green と判定される画像で、テンプレマッチ高スコアの方を best として選ぶ
- **`green_adoptable` の row=1 col=0 位置絶対化**: 同 uma に他の color=green box がない場合に限り、★0 / OCR 空でも row=1 col=0 を強制採用候補とする
- 緑名前テンプレマッチが Pass 0 から効くようになり、(空)系 4 件中 3 件が解消

### 残 Red 2 件（構造的限界）

| 画像 | role | 認識 | 期待 | 原因 |
|---|---|---|---|---|
| image0_test.png | parent1 | [スターライトビート]オグリキャップ | [キセキの白星]オグリキャップ | 継承タブ画像で緑因子=親由来→逆引き先衣装が自分と不一致 |
| receipt_1733 | parent2 | [超特急！フルカラー特殊PP]アグネスデジタル | [岩穿つ青]グラスワンダー | 緑名前テンプレ誤マッチ（精神一到→尊み☆ﾗｽﾄｽﾊﾟ）→ 逆引き連動誤 |

これらは画像内信号（タブ種別・テンプレ確信度）からの判別が困難。次の改修候補:

- 画像上部のタブテキスト（「ウマ娘詳細」「育成情報」）OCR で画像種別判別
- 緑名前テンプレに「精神一到何事か成らざらん」のサンプル増（現在 1 サンプル）
- character ONNX 再訓練（学習データ拡充）
...
- 受領 1558 parent1/parent2、1814 main/parent1 の緑 box は **col=1（白スロット位置）** にしか存在しない
- 緑アイコン付きのレース名スキル（「天皇賞（春）」等）を `detect_factor_color` が緑と誤判定してる
- 本来の緑因子タイル（恵福バルカローレ 等、col=0 側）は cropper 段階で検出されていない
- col=1 を緑候補から除外するだけではレース名行の OCR 結果が「(empty)」に変わるのみで誤認件数は不変
- 根本解決には cropper の色判定強化（緑アイコン付きレース名を除外）または緑 box の位置制約強化が必要

**Exp 10 の探索過程**:

1. PERTURBATIONS_RED の dy を下方向に拡張（dy=-3..+14）→ 効果なし。赤 ONNX は crop 位置を変えても「芝」を 0.9996 で返し続け、形状で判断している
2. bbox 一律 +6 シフト → 正位置画像 2 件（1832 main / sample_oguricap main）が悪化したため rollback
3. 正位置 + シフト版の ONNX アンサンブル → 正位置側の高 conf 誤答が max-picking で勝ち続け効果なし
4. **display_crop の bbox を y1 のみ +14（y0 は変更せず）** → 上の行を含まずテキストだけを OCR に入れる非対称 pad で、allowlist OCR が「マイル」「長距離」を拾えるようになり +2 件改善

同じ「y1 拡張」でも、ONNX 入力（27×175 形状）だと訓練ドメインから外れて効果なしだが、**OCR は拡張後の crop サイズを許容するので効く**点が肝。
...
- box.bbox は★中心基準で算出されており、縦 27px が因子名テキストの下端まで届かず、一部画像でテキストがフレーム外にはみ出す
- 青の場合、文字数が多く（「スピード」「スタミナ」）拡張で OCR が allowlist で拾える → 改善に直結
- 赤の場合、短い文字列（「マイル」「長距離」）で拡張すると上下の★や他の行が混入して OCR 誤読を起こす → rollback が必要だった
