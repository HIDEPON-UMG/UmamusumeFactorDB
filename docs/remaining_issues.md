...
最終更新: 2026-04-21（本日分の作業後に整理）

## 0. 今日までの到達点

| 指標 | 値 | メモ |
|---|---|---|
| ★精度 | **31/37 (83.8%)** | ベースライン 27/37 から +4 件 |
| 因子名精度 | **16/37 = 43%**（誤認 21 件） | 従来 24 件から −3 件 |
| 悪化 | **0 件** | 現 28 件の正解は全て維持 |
| Cloud Run リビジョン | `factor-processor-00015-29k` | commit `9131b0d` 反映済 |
| 主要改修 | CNN 分類器導入 / row 0 位置絶対化 / rank fallback ガード / 赤因子 OCR allowlist | — |
...
### C. 色判定ミス（★スロットに到達しない）— 1 件

| # | image | role / color | 現 → 正解 | 真因 |
|---|---|---|---|---|
| C1 | sample_oguricap.png | main / green | ★0 → ★2 | 緑因子行の color が `white` 判定されて緑スロットに流れない |

**対策候補**:
- D1. `detect_factor_color` の閾値チューニング（ratio_in_range > 0.25 → 0.20）
...
### Plan 1: 低リスク・高効果（半日〜1 日）— **推奨**

1. **F1. 青因子 allowlist OCR 実装**（赤因子と同じパターン）
2. **E4. 低 confidence レビューキュー化**（誤書き込み回避、精度メトリクス自体は不変）
3. **D1. 色判定閾値チューニング**（sample_oguricap 単独救済）
4. **G2. rapidfuzz 重み調整**（緑因子誤マッチの緩和）

**期待効果**: 青 +1〜2 件、緑 +2〜3 件、合計 +3〜5 件

### Plan 2: 中リスク・中効果（1-2 日）
...
1. **Plan 1**（青 allowlist + 緑 fuzzy 調整）を先行実施 — リスク低 / 実装短時間 / 複数件を一度に狙える
2. **Plan 2**（ONNX 再訓練）— 赤因子 5 件の解消に直結、中期投資
