...
### 3.1 テンプレ tar.gz の再生成（毎回必須）

Cloud Run の container import は image レイヤーに **Unicode 名のディレクトリ
（日本語/☆等）** が含まれていると `ContainerImageImportFailed` で拒否する。
そのため `datasets/{red_blue,star,green_name}_templates/` を直接 COPY せず、
ASCII 名の単一 tar.gz として送り込み起動時に展開している。

deploy 直前に必ず tar.gz を再生成する：

```bash
.venv/Scripts/python.exe scripts/build_template_tarball.py
```

これで `datasets/all_templates.tar.gz` が更新される。templates の中身を変えた
場合は再生成しないと古いテンプレでビルドされる点に注意。
...
```bash
curl https://factor-processor-xxxxx-an.a.run.app/health
```

期待応答：`{"ok":true,"target_tab":"factors_normalized","has_webhook":true}`

> Cloud Run の Google Front-End は `/healthz` パスを独自に 404 としてしまう
> （内部用の予約パスらしい）。アプリ側では `/health` と `/healthz` の両方を
> 提供しているが、外部からチェックするときは必ず `/health` を使うこと。
